...
class Converter(QThread):
    ...
    def convert(self, input_path, output_path, audio_settings, target_size_mb=None, resize_settings=None):
        try:
            ...
            if resize_settings:
                cmd.extend(['-vf', f"'{resize_settings[0]}'"])
            ...
            while True:
                ...
                stdout += process.stdout.read()  # Capture stdout
                stderr += line  # Capture stderr
...
class VideoConverter(QMainWindow):
    ...
    def set_preset_resolution(self):
        ...
        if preset_text == "Select Preset":
            ...
            self.update_width_height_display(str(width), str(height))
    ...
    def get_resize_settings(self, input_path):
        ...
        if resize_filter:
            return [resize_filter]  # Gibt nur noch den Filter-String zurück
        else:
            return []
