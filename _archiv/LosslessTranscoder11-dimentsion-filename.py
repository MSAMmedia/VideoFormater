...
class Converter(QThread):
    ...
    def convert(self, input_path, output_path, audio_settings, target_size_mb=None, resize_settings=None):
        try:
            ...
            if resize_settings:
                cmd.extend(['-vf', f"'{resize_settings[1]}'"])  # Setze den Filter in einfache Anführungszeichen
            ...
            print("FFmpeg Command:", " ".join(cmd))  # Diese Zeile einfügen
...
class VideoConverter(QMainWindow):
    ...
    def set_preset_resolution(self):
        ...
        if preset_text == "Select Preset":
            ...
            self.update_width_height_display(str(width), str(height)) # Diese Zeile wurde hinzugefügt
    ...
    def get_resize_settings(self, input_path):
        ...
        if aspect_ratio_handling == "Maintain Aspect Ratio (Black Bars)":
            ...
            resize_filter = ""
            QMessageBox.warning(self, "Warning", "Invalid aspect ratio handling selected.") # Warnmeldung
            return []

        if resize_filter:
            return ['-vf', resize_filter]  # Gib nur den Filter-String zurück
        else:
            return []
    ...
    def start_conversion(self):
        ...
        for i in range(self.file_list.count()):
            ...
            if metadata:
                parts = [file_name]
                if metadata.video_codec != 'N/A':
                    parts.append(f"[{metadata.video_codec.upper()}]")
                if metadata.audio_codec != 'N/A':
                    parts.append(f"[{metadata.audio_codec.upper()}]")
                if metadata.resolution != 'N/A':
                    parts.append(f"[{metadata.resolution}]")
                if metadata.bitrate != 'N/A':
                    parts.append(f"[{int(float(metadata.bitrate) / 1000)}kbps]")
                parts.append(f"[{metadata.format_duration()}]")
            ...
            # ... (Rest des Codes in start_conversion) ...
