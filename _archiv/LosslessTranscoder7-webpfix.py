...
from PyQt6.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QCheckBox, QGroupBox, QHeaderView, QSizePolicy, QMainWindow, QMenuBar, QMenu, QDialog, QVBoxLayout, QTextBrowser
)
...
class Converter(QThread):
    ...
    def convert(self, input_path, output_path, audio_settings):  # Added audio_settings parameter
        ...
        try:
            ...
            if output_path.lower().endswith('.webm'):
                cmd.extend([
                    ...
                    '-b:v', '2M',  # Video bitrate (adjust if needed)
                    '-c:a', 'libvorbis',  # Use Vorbis audio codec
                    '-b:a', '128k'  # Audio bitrate (adjust if needed)
                ])
            else:
                cmd.extend([
                    '-c:v', 'copy'  # Keep video, only change container
                ])
                cmd.extend(audio_settings)  # Use audio setting
